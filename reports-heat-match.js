(() => {
  ...
  function ensureStyles(){
    ...
    style.textContent=`
      ...
      .report-zero-zone-layout .zone-t1{grid-column:2;grid-row:1}.report-zero-zone-layout .zone-t2{grid-column:3;grid-row:1}.report-zero-zone-layout .zone-l1{grid-column:1;grid-row:2}.report-zero-zone-layout .zone-l2{grid-column:1;grid-row:3}.report-zero-zone-layout .zone-r1{grid-column:4;grid-row:2}.report-zero-zone-layout .zone-r2{grid-column:4;grid-row:3}.report-zero-zone-layout .zone-b1{grid-column:2;grid-row:4}.report-zero-zone-layout .zone-b2{grid-column:3;grid-row:4}.report-zero-zone-layout .core-grid{grid-column:2/4;grid-row:2/4;display:grid;grid-template-columns:1fr 1fr;grid-template-rows:1fr 1fr;gap:3px}
  ...
  function applyGameHeatColors(root){const active=root.querySelector('[data-heat-result].active'),result=active?.dataset.heatResult||'ALL',color=COLORS[result]||'#101011',zones=[...root.querySelectorAll('.report-zone-layout .heat-zone')];if(!zones.length)return;const values=zones.map(zone=>Number.parseFloat(zone.querySelector('.pct')?.textContent)||0),distinct=[...new Set(values.filter(v=>v>0))].sort((a,b)=>a-b),n=parseInt(color.slice(1),16),light=((n>>16)*299+((n>>8)&255)*587+(n&255)*114)/1000;zones.forEach((zone,index)=>{const value=values[index];if(!value){zone.style.background='#edf2ef';zone.style.color='#667085';return}const rank=distinct.indexOf(value),strength=distinct.length===1?1:.15+.85*(rank/(distinct.length-1));zone.style.background=strength===1?color:mixHexWithWhite(color,strength);zone.style.color=strength>.62&&light<155?'#fff':'#111'})}
  function zeroHeatChart(){const cell=(z,core='')=>`<div class="zone heat-zone ${core} zone-${z.toLowerCase()}"><span class="pct">0</span></div>`,div=document.createElement('div');div.className='report-zone-layout report-zero-zone-layout';div.innerHTML=['T1','T2','L1','L2'].map(z=>cell(z)).join('')+`<div class="core-grid">${['C1','C2','C3','C4'].map(z=>cell(z,'core')).join('')}</div>`+['R1','R2','B1','B2'].map(z=>cell(z)).join('');return div}
  function keepEmptyHeatCharts(){document.querySelectorAll('.report-heat').forEach(heat=>{if(heat.querySelector('.report-zone-layout'))return;const empty=[...heat.querySelectorAll('p,div')].find(el=>/^No pitch-location data/i.test((el.textContent||'').trim())&&!el.querySelector('*'));if(empty)empty.replaceWith(zeroHeatChart())})}
  function simplifyReport(){
    ...
    document.querySelectorAll('.report-spray-box').forEach(box=>{
      let title=box.querySelector(':scope > .report-section-label');
      const previous=box.previousElementSibling;
      if(!title&&previous?.classList.contains('report-section-label')&&previous.textContent.trim()==='Spray Chart'){title=previous;box.prepend(title)}
      if(!title){title=document.createElement('h3');title.className='report-section-label';title.textContent='Spray Chart';box.prepend(title)}
    });
    document.querySelectorAll('.report-heat>h3').forEach(title=>{if(title.textContent!=='Heat Chart')title.textContent='Heat Chart'});
  }
