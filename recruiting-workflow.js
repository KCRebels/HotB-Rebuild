(function(root,factory){
 ...
 function init(){
  ...
  if(root.firebase?.auth)root.firebase.auth().onAuthStateChanged(()=>setTimeout(()=>{const name=document.querySelector('#evalSelect')?.value;if(name&&name!=='Team')publishProfile(name,false)},200));
  inject();
 }
 if(typeof document!=='undefined'&&document.readyState==='loading')document.addEventListener('DOMContentLoaded',init);else init();
 return {PROFILE_BASE_URL,DEFAULT_EVALUATION,EVENTS,activePlayer,buildEmailSubject,buildPlayerEmailBody,shortCoachIntro,coachMatches,payload,slugFor,profileUrl};
});