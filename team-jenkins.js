...
function seed(){const db=read();if(!db||!Array.isArray(db.roster))return;let changed=false;players.forEach(profile=>{let p=db.roster.find(x=>x.name===profile.name);if(!p){db.roster.push({...profile,side:'R',isGuest:true,isTeamJenkins:true,teamName:TEAM});changed=true}else{const next={...p,...profile,isTeamJenkins:true,teamName:TEAM,isPracticeGuest:false};if(JSON.stringify(next)!==JSON.stringify(p)){Object.assign(p,next);changed=true}}});if(changed)write(db)}
