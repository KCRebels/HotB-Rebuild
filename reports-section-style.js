(() => {
  ...
  function ensureStyles(){
    ...
    s.textContent=`
      ...
      .report-standard-section .hotb-count-header{display:flex!important;align-items:center!important;justify-content:space-between!important;gap:8px!important;width:100%!important;margin:0 0 12px!important;padding:0!important;border:0!important;background:transparent!important;box-shadow:none!important}
      ...
      .report-standard-section .hotb-count-header .count-key,.report-standard-section .hotb-count-header .eval-count-key{margin-left:auto!important;flex:0 0 auto!important;font-size:18px!important;white-space:nowrap!important}
      .report-standard-section-title-row{display:flex!important;align-items:center!important;gap:8px!important;width:100%!important;margin:0 0 12px!important}
      .report-standard-section-title-row .report-standard-section-title{margin:0!important;flex:0 0 auto!important}
      .report-standard-section-subtitle{margin-left:auto!important;flex:1 1 auto!important;text-align:left!important;font-size:15px!important;line-height:1.1!important;font-weight:900!important;color:#111!important;white-space:normal!important}
      @media(max-width:560px){
        ...
        .report-standard-section .hotb-count-header .count-key,.report-standard-section .hotb-count-header .eval-count-key{font-size:16px!important}
        .report-standard-section-subtitle{font-size:14px!important}
      }
    `;
  }
  ...
  function plainTitle(el){
    if(!el)return;
    el.classList.add('report-standard-section-title');
    ['background','background-color','border','border-radius','padding','box-shadow','color','display','width'].forEach(p=>el.style.removeProperty(p));
  }

  function fixCount(){
    const c=title('COUNT PERFORMANCE');if(!c)return;
    plainTitle(c);
    const header=c.closest('.hotb-count-header');
    if(header){
      const key=header.querySelector('.count-key,.eval-count-key');
      if(key){header.style.setProperty('display','flex','important');header.style.setProperty('justify-content','space-between','important');header.style.setProperty('gap','8px','important')}
    }
  }
  ...
  function fixStrike(){
    ...
    let subtitle=null;
    const candidates=[...box.querySelectorAll('span,div,b,strong,p')].filter(el=>el!==k&&!el.contains(k));
    subtitle=candidates.find(el=>/^\(\d+\)\s*\(COUNT\)\s*\(TOTAL PITCHES\)$/i.test(el.textContent.trim()));
    if(!subtitle)return;
    const row=document.createElement('div');row.className='report-standard-section-title-row';
    k.parentNode.insertBefore(row,k);row.appendChild(k);row.appendChild(subtitle);subtitle.classList.add('report-standard-section-subtitle');
  }

  function fixSimple(text){const el=title(text);if(el)plainTitle(el)}
  ...
  function apply(){
    ...
    fixCount();fixStrike();fixSimple('SPRAY CHART');fixSimple('HEAT CHART');
  }
