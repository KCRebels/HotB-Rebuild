(()=>{
 ...
 function hybridize(){
  ...
  if(!preview||!name||!address)return;
  lastCoachName=name.value;
  if(document.querySelector('#rwHybridArea'))return;
  ...
  const sync=()=>{lastCoachName=name.value;const next=newBody(lastCoachName);if(!body.value||body.value===auto){body.value=next;auto=next}send.disabled=!clean(address.value)||!address.validity.valid};
  ['input','change'].forEach(type=>{name.addEventListener(type,()=>setTimeout(sync,0));address.addEventListener(type,()=>setTimeout(sync,0))});
  send.addEventListener('click',()=>{if(!clean(address.value)||!address.validity.valid)return;pendingBody=body.value||newBody(lastCoachName);preview.click()});
  sync();
 }
 function applyPreview(){
  const textarea=document.querySelector('#emailBodyPreview');
  if(!textarea)return;
  const replacement=pendingBody||newBody(lastCoachName);
  if(textarea.dataset.brooklynTemplate!=='1'){textarea.value=replacement;textarea.dataset.brooklynTemplate='1'}
  if(pendingBody){pendingBody='';setTimeout(()=>document.querySelector('#openGmailDraft')?.click(),0)}
 }
