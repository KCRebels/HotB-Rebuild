(()=>{
 ...
 function hybridize(){
  ...
  send.addEventListener('click',()=>{
   if(!clean(address.value)||!address.validity.valid)return;
   const hidden=document.createElement('button');hidden.type='button';hidden.style.display='none';hidden.id='rwHybridPreviewProxy';document.body.appendChild(hidden);
   const original=document.createElement('button');original.id='previewRecruitingEmail';original.style.display='none';area.appendChild(original);
   original.addEventListener('click',()=>{});
   original.click();
  });
  sync();
 }
 const obs=new MutationObserver(()=>requestAnimationFrame(()=>{wireButton();hybridize()}));
 obs.observe(document.getElementById('app')||document.body,{childList:true,subtree:true});
 document.addEventListener('change',e=>{if(e.target?.id==='evalSelect')setTimeout(wireButton,0)});
 wireButton();hybridize();
})();
