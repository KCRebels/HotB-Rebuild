...
function repairFrontToss(plan){
 ...
 function solve(fourUsed=false){
  ...
  for(const c of candidates){
   const makesFour=c.n===3;groups[c.i].push(name);remaining.delete(name);
   const singles=groups.filter(g=>g.length===1).length;
   if(singles<=remaining.size&&solve(fourUsed||makesFour))return true;
   remaining.add(name);groups[c.i].pop();
  }
  return false;
 }
...
api.buildSchedule=function(players,start,duration,options){
 ...
 alert('Build Anyway could not create a legal Front Toss exception with this attendance. No schedule was changed.');
 return plan;
};
})();