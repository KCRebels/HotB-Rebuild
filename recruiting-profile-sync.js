...
const SWINGS=new Set(['F','K','HIT','H4O','E','FC','SAC']);
const CONTACT=new Set(['F','HIT','H4O','E','FC','SAC']);
...
function hitting(db){const games=[...(db.savedGames||[]),...(db.currentGame?[db.currentGame]:[])].filter(currentSeasonGame);const pas=games.flatMap(g=>g.plateAppearances||g.pas||[]).filter(pa=>pa.hitter===PLAYER);const stats=window.HotBEvaluationStats?.statsForPAs?.(pas);if(!stats)return null;const pitches=games.flatMap(g=>g.pitches||[]).filter(p=>p.hitter===PLAYER&&SWINGS.has(String(p.result||'').toUpperCase()));const contacts=pitches.filter(p=>CONTACT.has(String(p.result||'').toUpperCase())).length;return{PA:String(stats.PA),AVG:formatAvg(stats.AVG),OBP:formatAvg(stats.OBP),CONTACT:pitches.length?`${Math.round(contacts/pitches.length*100)}%`:'—'}}
