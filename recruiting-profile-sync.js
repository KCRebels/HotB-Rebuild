...
function currentSeason(){const d=new Date(),y=d.getFullYear(),m=d.getMonth()+1,day=d.getDate(),sy=(m>=9||m===8||(m===7&&day>=31))?y:y-1;return `${sy}–${String(sy+1).slice(-2)}`}
function currentSeasonGame(game){const meta=typeof window.HotBSeasonMeta==='function'?window.HotBSeasonMeta(game?.date):null;if(meta)return meta.season===currentSeason()&&meta.segment!=='Dead Period';const d=new Date(game?.date);if(Number.isNaN(d.getTime()))return false;const y=d.getFullYear(),m=d.getMonth()+1,day=d.getDate();if((m===7&&day>=31)||m===8)return false;const sy=m>=9?y:y-1;return `${sy}–${String(sy+1).slice(-2)}`===currentSeason()}
...
function payload(){const db=readDb(),hit=hitting(db),pitch=pitching(db);if(!hit||!pitch)return null;return{hitting:hit,pitching:pitch,performanceSeason:`${currentSeason()} full season`}}
async function sync(){if(busy||!coachUser||!store)return;const data=payload();if(!data)return;const signature=JSON.stringify(data);if(signature===lastSignature)return;busy=true;try{await store.collection('recruitingProfiles').doc(PROFILE_ID).set({...data,performanceSource:'HotB Eval',updatedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});lastSignature=signature}catch(e){console.warn('Recruiting profile sync deferred',e)}finally{busy=false}}
