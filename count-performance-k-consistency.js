(() => {
  function fixRow(row){
    ...
    if(!['0-0','0-2','1-2','2-2','3-2','6+'].includes(bucket))return;
    ...
    if(bucket==='0-0'){
      ...
      if(k){
        k.textContent='X';
        k.classList.remove('zero','positive');
        k.style.setProperty('color','#111','important');
      }
    }
  }
  function sync(){
    document.querySelectorAll('.count-card,.eval-count-card').forEach(fixRow);
  }
  ...
  // HotB replaces report/eval markup during render. A small targeted check keeps only
  // these twelve Count Performance rows correct without observing or rewriting the page.
