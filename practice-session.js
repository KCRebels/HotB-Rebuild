(function(root,factory){
 ...
 function restore(saved,now=Date.now()){
  ...
  session.clock={running:!!clock.running,finished:!!clock.finished,endAnnounced:!!clock.endAnnounced,startAt:Number(clock.startAt)||0,lastBlock:Number(clock.lastBlock)||1,lastTwoMinuteBlock:Number(clock.lastTwoMinuteBlock)||0,lastTransitionBlock:Number(clock.lastTransitionBlock)||0,completedAt:clock.completedAt||null,elapsedOffsetMs:Math.max(0,Number(clock.elapsedOffsetMs)||0)};
  if(session.plan&&session.clock.running&&session.clock.startAt){
   const blockMs=(Number(session.plan.blockMinutes)||12)*60000,elapsed=Math.max(0,Number(now)-session.clock.startAt+session.clock.elapsedOffsetMs),recoveredBlock=Math.max(1,Math.min(Number(session.plan?.times?.length)||10,Math.floor(elapsed/blockMs)+1));
 ...
 function timing(plan,clock,now=Date.now()){
  ...
  const {blockMs,workMs,totalMs,blockCount}=layout(plan),elapsed=Math.max(0,Number(now)-Number(clock.startAt)+Math.max(0,Number(clock.elapsedOffsetMs)||0));
