...
function bindPractice(){
 ...
 $('#openPracticeBuilder')?.addEventListener('click',()=>{if(db.activePortalPractice?.id&&!practicePlan){alert('A practice is still active on the player and coach portals. Resume and finish that practice before building a new one.');return}
  if(!practicePlan){
   // Opening Build Practice is the attendance default boundary: every current
   // roster player and any already-added practice guest starts checked.
   practiceSetupState.selectedNames=practiceAttendanceRoster().map(player=>player.name);
   practiceSection='setup';
   render();
   // Persist only after the checked inputs exist, so persistPracticeDraft cannot
   // read zero setup checkboxes from the hub and overwrite the default with [].
   setTimeout(()=>persistPracticeDraft(),0);
   window.scrollTo(0,0);
   return;
  }
