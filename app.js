...
function portalPlayerEvaluationView(){
 ...
 <section class="player-card player-profile" style="margin-top:12px">${player.grad?`<div class="grad-year">${esc(player.grad)}</div>`:''}<div class="player-photo">${player.photo?`<img src="${encodeURI(player.photo)}" alt="${esc(player.name)}">`:esc(player.name.split(' ').map(x=>x[0]).join(''))}</div><div class="player-info"><div class="name">${esc(player.name)}</div>${profileMeta?`<div class="meta">${profileMeta}</div>`:''}${player.interest||player.school?`<div class="interest">${esc(player.interest||'')} ${player.school?`<span>| ${esc(player.school)}</span>`:''}</div>`:''}</div></section>
