...
function bindReportsPage(){
 ...
 const scrimmageToggle=document.querySelector('[data-scrimmage-game]');if(scrimmageToggle)scrimmageToggle.onchange=()=>{const game=db.savedGames.find(item=>item.id===scrimmageToggle.dataset.scrimmageGame);if(!game)return;game.scrimmage=!!scrimmageToggle.checked;reportSelectedGameIds=reportSelectedGameIds.filter(id=>id!==game.id);if(reportGameId===game.id)reportGameId=null;save();schedulePlayerEvaluationPortalSync(0);render()};
