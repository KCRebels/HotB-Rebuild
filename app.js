...
function portalCoachView(){
 ...
 if(!cloudUser)return `${portalHeader()}<main class="portal-page"><section class="portal-welcome"><span>COACH SETUP</span><h2>Opening Player Portal Manager</h2><p>HotB is reconnecting this device to the portal service. Your saved player links have not been removed.</p></section><button class="btn black block" id="retryPortalCloud">Reconnect</button><button class="btn block" data-go="home">Return Home</button></main>`;
...
function bindPlayerPortal(){
 ...
 $('#retryPortalCloud')?.addEventListener('click',()=>{cloudInitStarted=false;cloudInitRetryCount=0;initCloud();});
