...
function playerEvaluationPortalPayload(playerName){
 ...
 return JSON.parse(JSON.stringify({roster:[pick(player,rosterFields)],savedGames:games,currentGame:null,evaluationSeason:season,evaluationFilterLabel,playerMetrics:{hotB:playerMetrics.hotB},published}));
}
...
function portalPlayerEvaluationView(){
 ...
 const fmtAvg=value=>displayStats.PA?Number(value||0).toFixed(3).replace(/^0/,'.'):'—',fmtPct=value=>stats.PA?`${Math.round(Number(value||0)*100)}%`:'—';
 ...
 <section class="portal-eval-section"><div class="portal-eval-section-head"><h3>Hitting Results</h3><span>${esc(payload.evaluationFilterLabel||payload.evaluationSeason||'')} · ${displayStats.PA} PA · ${published?'COACH':'LEGACY'}</span></div><div class="portal-eval-grid">${metric('HotB+',hotbValue==null?'—':Math.round(hotbValue))}${metric('RP',rp==null?'—':rp.toFixed(1))}${metric('AVG',fmtAvg(displayStats.AVG))}${metric('OBP',fmtAvg(displayStats.OBP))}${metric('K%',fmtPct(displayStats.kPct))}${metric('Contact%',fmtPct(displayStats.contactPct))}${metric(resultLabel,fmtPct(resultValue))}</div><p class="portal-eval-private">These are your results only. Player rankings and roster comparisons are not available in the player portal.</p></section>
