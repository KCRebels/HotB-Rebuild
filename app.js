...
async function recoverOrphanedActivePractice(){
 ...
 try{
  ...
  const recoveredPlayerSchedules={};for(const item of remote.players||[]){const source=rosterByFirst.get(item.name),name=source?.name||activeNameByFirst.get(item.name)||item.name;recoveredPlayerSchedules[name]=structuredClone(item.schedule)}
