  ...
  if(practicePlan.feasibilityErrors?.length){
   ...
   const runResolutionCandidates=(candidates,stage,buildCandidate,onSafe,ownershipMessage,stopAfterFirst=false)=>{
    ...
    for(let index=0;index<unique.length;index++){
     ...
     const diagnosticBase={bundle:'resolution436',stage,label:spec.label,index:index+1,total:unique.length,sourceCandidates:candidates.length,startedAt:new Date().toISOString()};
