...
if((db.teamJenkinsDataCleanupVersion||0)<2){
 ...
 if(jenkinsNames.size){
  ...
  (db.practiceHistory||[]).forEach(record=>{if(Array.isArray(record.attendees))record.attendees=record.attendees.filter(name=>!jenkinsNames.has(name))});
...
async function syncPlayerPracticeClock(){
 ...
 try{
  ...
  practiceGuestPlayers().filter(guest=>attending.has(guest.name)&&guest.portalId).forEach(guest=>batch.set(portalDoc(guest.portalId),{'activePractice.clock':clock,updatedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true}));
...
function bindPractice(){
 ...
 $('#startPracticeClock')?.addEventListener('click',()=>{
  if(practiceClock.finished)return;
  if(practiceClock.running&&!confirm('Restart this practice from Block 1? The current practice clock will be reset.'))return;
  beginPracticeClock();
 });
