...
function portalPlayerEvaluationView(){
 ...
 if(payload&&playerName){const portalPlayer=(db.roster||[]).find(p=>p.name===playerName)||null;window.__HOTB_PORTAL_PLAYER_NAME__=playerName;window.__HOTB_PORTAL_EVAL_DB__={...payload,roster:portalPlayer?[JSON.parse(JSON.stringify(portalPlayer))]:(payload.roster||[]),measurements:(db.measurements||[]).filter(m=>m.player===playerName)};}
