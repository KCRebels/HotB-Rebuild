...
function playerEvaluationPortalPayload(playerName){
 ...
 const playerPas=games.flatMap(game=>game.plateAppearances||[]),playerMetrics=HotBEvaluationStats.hotBMetrics(playerPas,teamPas);
 return JSON.parse(JSON.stringify({roster:[pick(player,rosterFields)],savedGames:games,currentGame:null,evaluationSeason:season,evaluationFilterLabel,playerMetrics:{hotB:playerMetrics.hotB}}));
}
