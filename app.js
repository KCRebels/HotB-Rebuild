...
function coachEvaluationPortalPayload(){
 ...
 const payload={roster:db.roster.filter(player=>!player.isGuest).map(player=>Object.fromEntries(fields.map(key=>[key,player[key]??'']))),savedGames:db.savedGames.map(game=>({id:game.id,date:game.date,opponent:game.opponent,plateAppearances:game.plateAppearances||[],pitches:(game.pitches||[]).map(pitch=>pick(pitch,pitchFields))})),measurements:(db.measurements||[]).map(({id,player,type,value,date})=>({id,player,type,value,date})),coaches:(db.coaches||[]).map(({coachName,coachEmail,collegeName,lastUpdated})=>({coachName,coachEmail,collegeName,lastUpdated})),practiceHistory:window.HotBPracticeHistory?.records(db.practiceHistory)||[],currentGame:null};
...
function bindPlayerPortal(){
 ...
 $$('[data-portal-practice-drill]').forEach(button=>button.addEventListener('click',()=>{if(!button.dataset.portalPracticeDrill)return;portalLibraryReturnView='practice';portalSelectedDrill=button.dataset.portalPracticeDrill;portalView='library';render();window.scrollTo(0,0)}));
...
function bindPractice(){
 ...
 $('#choosePracticeDrills')?.addEventListener('click',()=>{practiceDraftDrills=practiceChosenDrills.slice(0,practicePlan.drillStations);practiceDrillPickerOpen=true;practicePickerQuery='';practicePickerCategory='All Drills';persistPracticeSession();render();window.scrollTo(0,0)});
 $('#cancelPracticeDrills')?.addEventListener('click',()=>{practiceDraftDrills=[];practiceDrillPickerOpen=false;persistPracticeSession();render();window.scrollTo(0,0)});
 ...
 $('#practiceSelectAll')?.addEventListener('click',()=>{$$('[data-practice-player]').forEach(input=>input.checked=true);persistPracticeDraft()});
 $('#practiceSelectNone')?.addEventListener('click',()=>{$$('[data-practice-player]').forEach(input=>input.checked=false);persistPracticeDraft()});
 $$('[data-practice-player]').forEach(input=>input.addEventListener('change',persistPracticeDraft));
