...
function bindPractice(){
 ...
 $('#generatePractice')?.addEventListener('click',async()=>{
  ...
  if(practicePlan.feasibilityErrors?.length){
   ...
   const rosterGuidance=identityBlocked?'HotB found attendee identity or availability information that must be corrected before resolution. Fix the roster/guest or arrival/departure entry and build again; HotB will not guess or silently normalize it.':resolutionAuditFailures.length&&!hasVerifiedResolution?'HotB checked the safe automatic Block 11 alternative and it did not resolve this practice. Return to setup and make an explicit attendance, availability, Pitching, or Catching change; HotB will not make a role change speculatively.':availablePitchers.length?'If Block 11 alone does not solve the practice, return to setup and make the coaching change explicitly. HotB will not choose a hitter to remove or silently change Pitching/Catching.':'HotB needs a change to attendance or availability before it can satisfy every absolute rule.';
