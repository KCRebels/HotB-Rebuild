...
function bind(){
 ...
 if(modal==='practiceResolution'){
  ...
  $('#applyPracticeExtensionResolution')?.addEventListener('click',()=>{alert('HotB will only offer Block 11 after it has verified that the 11-block schedule solves this exact practice.');});
...
function bindPractice(){
 ...
 $('#generatePractice')?.addEventListener('click',()=>{
  ...
  if(practicePlan.feasibilityErrors?.length){
   ...
   const rosterGuidance=availablePitchers.length?'If HotB cannot prove a one-practice pitching change solves it, change attendance or availability here. HotB will not choose a hitter to remove.':'HotB needs a change to attendance or availability before it can satisfy every absolute rule.';
   practiceResolution={errors,pitchers:solvingPitchers,canExtend:false,rosterGuidance,practicePlayers,startTime,durationMinutes,noPitchersMode,notices:practicePlan.fallbackWarnings||[]};
