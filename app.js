...
function portalCoachView(){
 ...
 if(!cloudUser)return `${portalHeader()}<main class="portal-page"><section class="portal-welcome"><span>COACH SETUP</span><h2>Reconnect Player Portal Manager</h2><p>Your saved player links have not been removed. The coach cloud session on this device needs to be reconnected.</p></section><button class="btn black block" id="openPortalCloudSignIn" onclick="window.location.hash='cloud-backup'">Reconnect Cloud Backup</button><button class="btn block" data-go="home">Return Home</button></main>`;
