...
async function recoverOrphanedActivePractice(){
 ...
 try{
  ...
  const players=(remote.players||[]).map(item=>{const source=rosterByFirst.get(item.name),name=source?.name||activeNameByFirst.get(item.name)||item.name,base=practicePlayerModel(source||{name,positions:item.role||''}),published=(item.schedule||[]).map(entry=>recoveryAssignmentToEntry(entry.assignment)),present=published.map((entry,index)=>entry.activity!=='Not Present'?index:-1).filter(index=>index>=0),availableFromBlock=present.length?Math.min(...present):0,availableUntilBlock=present.length?Math.max(...present)+1:0;return {...base,name,isPitcher:base.isPitcher||/P/.test(item.role||''),isCatcher:base.isCatcher||/C/.test(item.role||''),canPitch:base.isPitcher||/P/.test(item.role||''),requiresPitchWarmup:base.isPitcher||/P/.test(item.role||''),canCatch:base.isCatcher||/C/.test(item.role||''),availableFromBlock,availableUntilBlock}});
  const schedule={};
