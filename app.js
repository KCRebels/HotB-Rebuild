...
async function recoverOrphanedActivePractice(){
 ...
 try{
  ...
  const startTime=parseLabel(startLabel),times=Array.from({length:10},(_,index)=>{const base=practiceTimeMinutes(startTime)+index*blockMinutes;return {block:index+1,start:practiceTimeLabel(practiceTimeValue(base)),end:practiceTimeLabel(practiceTimeValue(base+blockMinutes))}});
  const rosterByFirst=new Map(db.roster.map(player=>[practiceFirstName(player.name),player]));
  const activeNameByFirst=new Map((state.players||[]).map(name=>[practiceFirstName(name),name]));
  const players=(remote.players||[]).map(item=>{const source=rosterByFirst.get(item.name),name=source?.name||activeNameByFirst.get(item.name)||item.name,base=practicePlayerModel(source||{name,positions:item.role||''}),roleTokens=String(item.role||'').toUpperCase().split(/[^A-Z]+/).filter(Boolean),rolePitcher=roleTokens.includes('P'),roleCatcher=roleTokens.includes('C'),published=(item.schedule||[]).map(entry=>recoveryAssignmentToEntry(entry.assignment)),present=published.map((entry,index)=>entry.activity!=='Not Present'?index:-1).filter(index=>index>=0),availableFromBlock=present.length?Math.min(...present):0,availableUntilBlock=present.length?Math.max(...present)+1:0;return {...base,name,isPitcher:base.isPitcher||rolePitcher,isCatcher:base.isCatcher||roleCatcher,canPitch:base.isPitcher||rolePitcher,requiresPitchWarmup:base.isPitcher||rolePitcher,canCatch:base.isCatcher||roleCatcher,availableFromBlock,availableUntilBlock}});
  const schedule={};
  (remote.players||[]).forEach(item=>{const source=rosterByFirst.get(item.name),name=source?.name||activeNameByFirst.get(item.name)||item.name;schedule[name]=(item.schedule||[]).map(entry=>recoveryAssignmentToEntry(entry.assignment))});
  ...
  const recoveredPlan={portalDraftId:remote.id,startTime,durationMinutes,blockMinutes,times,players,schedule,blocks,drillStations,liveSessions,machineFocus,frontTossFocus,recoveredCoachSchedule:structuredClone(remote.schedule),warnings:['Recovered from the activated coach portal without rebuilding the scheduler.']};
  ...
  if(recoveredClock.running&&!window.HotBPracticeSession?.timing(recoveredPlan,recoveredClock,Date.now())){recoveredClock.running=false;recoveredClock.finished=true;recoveredClock.completedAt=clock.endedAt||new Date(recoveredClock.startAt+window.HotBPracticeSession.layout(recoveredPlan).totalMs).toISOString()}
