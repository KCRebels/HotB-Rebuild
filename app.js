...
function playerEvaluationPortalPayload(playerName){
 ...
 const playerPas=games.flatMap(game=>game.plateAppearances||[]),seasonTeamPas=sourceGames.flatMap(game=>game.plateAppearances||[]).filter(pa=>competitionRoster().some(item=>item.name===pa.hitter)),playerMetrics=HotBEvaluationStats.hotBMetrics(playerPas,seasonTeamPas);
 return JSON.parse(JSON.stringify({roster,savedGames:games,currentGame:null,evaluationSeason:season,playerMetrics:{hotB:playerMetrics.hotB}}));
}
...
function portalPlayerEvaluationView(){
 ...
 const hotbValue=Number.isFinite(Number(payload.playerMetrics?.hotB))?Number(payload.playerMetrics.hotB):null,rp=stats.PA?stats.rp:null,decision=snapshot.decision?.rate??snapshot.decision?.decisionRate??snapshot.decision?.percentage??null;
 const resultLabel=slap?'IPA%':'HHB%',resultValue=slap?stats.ipaPct:stats.hhbPct,metric=(label,value)=>`<div class="portal-eval-metric"><b>${esc(value)}</b><span>${esc(label)}</span></div>`;
 const fmtAvg=value=>stats.PA?Number(value||0).toFixed(3).replace(/^0/,'.'):'—',fmtPct=value=>stats.PA?`${Math.round(Number(value||0)*100)}%`:'—';
 const countBuckets=['0-0','0-2','1-2','2-2','3-2','6+'],counts=countBuckets.map(bucket=>snapshot.countPerformanceByBucket?.[bucket]||{bucket,H:0,H4O:0,K:0,AVG:0});
 const pitchNames={FB:'Fastball',CH:'Changeup',RS:'Rise',DP:'Drop',CV:'Curve',SC:'Screw'},pitchRows=Object.entries(pitchNames).map(([key,label])=>{const row=snapshot.pitchPerformanceByType?.[key];if(!row?.n)return'';return`<div class="portal-eval-row"><span><b>${esc(label)}</b><small>${row.n} pitches</small></span><span>Contact <b>${row.contactRate==null?'—':Math.round(row.contactRate*100)+'%'}</b></span></div>`}).join('');
 const heat=snapshot.heatByResult?.ALL||[],heatTotal=heat.reduce((sum,value)=>sum+Number(value||0),0),heatZones=['T1','T2','L1','L2','C1','C2','C3','C4','R1','R2','B1','B2'];
 return `${portalHeader('My Evaluation',true)}<main class="portal-page portal-player-evaluation"><section class="portal-welcome"><span>MY EVALUATION</span><h2>How I’m Doing</h2><p>Your private HotB results from the current season. Only your information is shown here.</p></section>
 <section class="portal-eval-section"><div class="portal-eval-section-head"><h3>Hitting Results</h3><span>${stats.PA} PA</span></div><div class="portal-eval-grid">${metric('HotB+',hotbValue==null?'—':Math.round(hotbValue))}${metric('RP',rp==null?'—':rp.toFixed(1))}${metric('AVG',fmtAvg(stats.AVG))}${metric('OBP',fmtAvg(stats.OBP))}${metric('K%',fmtPct(stats.kPct))}${metric('Contact%',fmtPct(stats.contactPct))}${metric(resultLabel,fmtPct(resultValue))}</div><p class="portal-eval-private">These are your results only. Player rankings and roster comparisons are not available in the player portal.</p></section>
 <section class="portal-eval-section"><div class="portal-eval-section-head"><h3>Heat Chart</h3><span>${heatTotal} pitches</span></div>${heatTotal?`<div class="portal-eval-heat">${heatZones.map((zone,index)=>`<div><span>${zone}</span><b>${heat[index]||0}</b></div>`).join('')}</div>`:'<p class="portal-eval-empty">Not enough pitch-location data yet.</p>'}</section>
 <section class="portal-eval-section"><div class="portal-eval-section-head"><h3>Count Performance</h3></div><div class="portal-eval-list">${counts.map(row=>`<div class="portal-eval-row"><span><b>${row.bucket}</b><small>H ${row.H} · H4O ${row.H4O} · K ${row.K}</small></span><span>AVG <b>${(row.H+row.H4O+row.K)?Number(row.AVG||0).toFixed(3).replace(/^0/,'.'):'—'}</b></span></div>`).join('')}</div></section>
 <section class="portal-eval-section"><div class="portal-eval-section-head"><h3>Pitch Performance</h3></div><div class="portal-eval-list">${pitchRows||'<p class="portal-eval-empty">Not enough pitch-type data yet.</p>'}</div></section>
 <section class="portal-eval-section"><div class="portal-eval-section-head"><h3>Player Trends</h3></div><p class="portal-eval-empty">Your current-season results above use the same HotB evaluation data as your coach. Trend windows will appear here as more plate appearances are recorded.</p></section>
 </main>`;
}
